...
```{r}
activityday <- activityday %>%
  mutate(semana=isoweek(date), dia=wday(date, abbr = FALSE, week_start = getOption("lubridate.week.start", 1))) %>%
  relocate(Id, date, semana, dia)

activityhour <- activityhour %>%
  mutate(semana=isoweek(datetime), dia=wday(datetime, abbr = FALSE, week_start = getOption("lubridate.week.start", 1)), hora=hour(datetime)) %>%
  relocate(Id, date, semana, dia)
```
...
=======
>>>>>>> 68b1f442b10410d88a759f992e3ff5636e016073
```{r}
ggplot(activityhour) +
